...
def Tunable_Coupler(
    ...
    combiner.movex(
        splitter.ports["o3"].center[0] + htr_length + 40 - combiner.ports["o1"].center[0]
    )
    ...
    # ------------------------------------------------------------------ #
    x0 = float(splitter.ports["o3"].center[0])   # splitter output face X
    x1 = float(combiner.ports["o1"].center[0])   # combiner input face X

    y_s2 = float(splitter.ports["o3"].center[1])  # upper arm start Y
    y_s3 = float(splitter.ports["o4"].center[1])  # lower arm start Y
    ...
    p_upper += gf.path.straight(length=vert )          # ↓ 20 µm south
    p_upper += gf.path.arc(radius=R, angle=90)        # → turn east
    ...
    p_lower += gf.path.straight(length=vert + extra )  # ↑ north (longer)
    p_lower += gf.path.arc(radius=R, angle=-90)       # → turn east
    ...
    c.add_port('e1', port = mt_patch_left.ports['e4'])
    c.add_port('e2', port = mt_patch_right.ports['e4'])
